...
def create_sample_question(schema_data, data):
    ...
    prompt = f"""
        Task: Generate me 50 questions based on data using the schema {schema_data} use
        summary statistics: {summary_statistics} and
        correlation statistics: {corr_data}. to generate more questions
    ...
    system = """
            You are a senior data analyst.Stricktly return a JSON object containing the List  of questions in this format `{questions:['question1','question2',...]}`. Do not provide explanations or notes.
        """
    response = run_mistral(prompt, system=system, output_format="json")
    ...
    try:
        ...
        logging.info(f"Question1: {question_1} -- Question2: {question_2} ")
    except:
        question_1 = None
        question_2 = None
        question_3 = None
...
#     goals_sample_question = re.findall(
#         r"<goal_start>(.*)<goal_end>", regenerate_new_question.replace("\n", " ")
#     )

#     if len(goals_sample_question) > 0:
#         prompt = (
#             f"You are an data analyst, "
...
#             f"summary statistics: {summary_statistics} and"
#             f"correlation statistics: {corr_data}. to generate more questions"
#             f"IMPORTANT:Strivtly Put each question in <question_start> your generated question <question_end>."
#         )
...
#             "<question_start>(.*?)<question_end>", response.replace("\n", " ")
#         )
...
@st.cache_data(show_spinner=False)
def query_chart_recommendation(
    _data_schema, new_question, recommened_query, number_of_records, _sample_data
):
    ...
    try:
        chart_recommendation = (
            re.search(r"<chart_start>(.*)<chart_end>", response.replace("\n", " "))
            .group(1)
            .strip()
        )
        x_recommendation = (
            re.search(r"<x_var_start>(.*)<x_var_end>", response.replace("\n", " "))
            .group(1)
            .strip()
        )
        y_recommendation = (
            re.search(r"<y_var_start>(.*)<y_var_end>", response.replace("\n", " "))
            .group(1)
            .strip()
        )
        hue_recommendation = (
            re.search(r"<hue_var_start>(.*)<hue_var_end>", response.replace("\n", " "))
            .group(1)
            .strip()
        )
        title_recommendation = (
            re.search(r"<title_start>(.*)<title_end>", response.replace("\n", " "))
            .group(1)
            .strip()
        )
    except:
...
def recursion_batch(list_of_df, list_of_result, new_question, query_recommendation):
    ...
    if len(list_of_df) <= 10:
        if len(list_of_df) < 2:
            ...
            return recursion_batch(
                new_list, list_of_result, new_question, query_recommendation
            )
    else:
...
def recursive_summarizer_sub(list_of_response, list_of_result_response, new_question):
    ...
    if len(list_of_response) < 2:
        ...
        prompt = (
            f"Given the question is {new_question}."
...
def split_words_into_sublists(word_list, max_words_per_list):
    ...
    sublists = [
        joined_words[i : i + max_words_per_list]
        for i in range(0, len(joined_words), max_words_per_list)
    ]
...
def explain_result(query_recommendation, new_question, dataframe_new):
    ...
    response = recursion_batch(
        list_of_df, list_of_result, new_question, query_recommendation
    )
    ...
    if response:
        ...
        if len(response) >= 2:
            ...
            response = recursive_summarizer_sub(
                sublists, list_of_result_response, new_question
            )
            response = "\n".join(response)
        else:
