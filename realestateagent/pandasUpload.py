...
st.set_page_config(
    page_title="Neuralytics - Real Estate",
    page_icon=":robot:",
    layout="wide",
...
with col_main_2:
    st.markdown("# **Neuralytics - Real Estate**")
    st.markdown(":red[Neuron-Q]")
    st.markdown(
        """
...
with col_main_2:
    if not UPLOADED_FILE:
        UPLOADED_FILE = "realestateagent/data_prices_cleaned.csv"
        st.markdown(
            "*Application is currently running the sample dataset. To get insights from your own data, please upload your csv file.*"
        )
...
def convert_datatype(df):
    ...
    for c in df.columns[df.dtypes == "object"]:
        try:
            ...
            df[c] = df[c].apply(str.lower) if df[c].dtype == "str" else df[c]
...
def get_data_overview(header):
    prompt = (
        f"Format your answer to markdown latex. Use markdown font size 3. "
...
def get_dataframe_from_duckdb_query(query):
    try:
        ...
        """
        response = agent.gpt_promt_davinci(prompt)
        try:
            query = (
                re.search(r"<sql_start>(.*)<sql_end>", response.replace("\n", " "))
                .group(1)
                .strip()
            )
            dataframe_new = duckdb.query(query).df()
...
def query_text(_schema_data, new_question, _sample_data):
    ...
    # Get the query
    query_recommendation = re.sub(
        " +",
        " ",
        agent.generate_sql_gpt(schema_data, new_question, _sample_data),
    )
    # Create the new dataframe
    dataframe_new, query_recommendation = get_dataframe_from_duckdb_query(
        query_recommendation
    )
    batch_size = round(len(dataframe_new.to_json()) / 3200) + (
        len(dataframe_new.to_json()) % 3200 > 0
    )
    ...
    ) = agent.query_chart_recommendation(
        schema_data_new,
        new_question,
        query_recommendation,
        len(dataframe_new),
    ...
    if len(dataframe_new) > 0:
        pass
        response = agent.explain_result(
            query_recommendation, new_question, dataframe_new
        )
        print("Response", response)
    else:
        response = agent.query_no_result(
            _schema_data, new_question, query_recommendation
        )
...
def show_messages(_index_generated, _index_past, _i, is_result):
    ...
    with st.expander(f"{str(_i+1)}.{st.session_state[_index_past][_i]}"):
        if is_result:
            message(
                (st.session_state[_index_generated][_i]).strip(),
                key=str(_i),
                avatar_style="thumbs",
                seed="Mimi",
            )
            ...
            message(
                "The query produce no result, please rephrase the question.",
                key=str(_i),
                avatar_style="thumbs",
                seed="Mimi",
            )
        message(
            st.session_state[_index_past][_i],
            is_user=True,
            key=str(_i) + "_user",
            avatar_style="thumbs",
            seed="Mia",
        ...
        index_q = next(
            (
                index
                for (index, d) in enumerate(st.session_state["all_result"])
                if d["question"] == st.session_state[_index_past][_i]
            ),
            None,
        )
        ...
        if tog.st_toggle_switch(
            label=f"Hide Graph",
            key=key_build,
            default_value=st.session_state["all_result"][index_q]["hide_graph"],
            label_after=False,
...
def ask_new_question(sample_question, schema_data, sample_data):
    ...
    if sample_question:
        new_question = (
            form.text_area(
                "Typing in your own question below...👇",
    ...
    with st.spinner("Analysing data..."):
        if (submit_button) or (sample_question):
            if new_question:
                if new_question not in st.session_state[index_questions]:
                    ...
                    for key in st.session_state[index_questions]:
                        if new_question == key:
                            ...
                            (
                                ...
                                title_recommendation,
                                query_recommendation,
                            ) = query_text(schema_data, key, sample_data)
                    ...
                    st.info(
                        "Question exists, bringing question to recent view...", icon="⚠️"
                    )
                    exist_question_index = st.session_state[index_past].index(
                        new_question
                    )
                    exist_question = st.session_state[index_past].pop(
                        exist_question_index
                    )
                    # print(f"This question exists: {exist_question}")
                    exist_output = st.session_state[index_generated].pop(
                        exist_question_index
                    )
            ...
            if st.session_state["all_result"]:
                ...
                if st.session_state[index_generated]:
                    ...
                    with placeholder.container():
                        total_length_reverse = reversed(
                            range(len(st.session_state[index_generated]) - 1, -1, -1)
                        )
                        for i in total_length_reverse:
                            try:
                                if (
                                    st.session_state[index_generated][i]
                                ).strip() == "The query produce no result, please rephrase the question.":
                                    ...
                                    if counter_non_result <= 1:
                                        ...
                                        show_messages(
                                            index_generated, index_past, i, False
                                        )
        ...
        with dashboard_col:
            with elements("dashboard"):
                ...
                if (
                    "streamlit_elements.core.frame.elements_frame.dashboard"
                    in st.session_state
                ):
                    if st.session_state[
                        "streamlit_elements.core.frame.elements_frame.dashboard"
                    ]:
                        session_state_layout = json.loads(
                            st.session_state[
                                "streamlit_elements.core.frame.elements_frame.dashboard"
                            ]
                        )
                        if (
                            "streamlit_elements.core.frame.elements_frame.dashboard00000000"
                            in session_state_layout
                        ):
                            layout = session_state_layout[
                                "streamlit_elements.core.frame.elements_frame.dashboard00000000"
                            ]["updated_layout"]
                    # print("============================================================================")
                ...
                with dashboard.Grid(layout, onLayoutChange=handle_layout_change):
                    ...
                    show_dashboard(
                        st.session_state["all_result"], index_question_counter
                    )
...
if UPLOADED_FILE is not None:
    ...
    with col_main_2:
        ...
        st.markdown(
            "The topic below gives you a general feel of the dataset, click on the expander to see more."
        )
        ...
        st.write(
            "Below are some sample questions, pick one of the questions below to see how our AI can analyse your question."
        )
        ...
        with col_question_1:
            if st.button("🔄 Re-generate sample question"):
                ...
                regenerate_new_question = "regenerate_sample_question" + str(
                    st.session_state["sample_question_generation"]
                )
        ...
        (
            ...
            sample_question_4,
            sample_question_5,
        ) = agent.create_sample_question(schema_data, DATA)
        print(sample_question_1)
        question = None

        # Create the sample questions columns
        with col1:
            if st.button(label=sample_question_1):
                question = sample_question_1.lower()

        with col2:
            if st.button(label=sample_question_2):
                question = sample_question_2.lower()

        with col3:
            if st.button(label=sample_question_3):
                question = sample_question_3.lower()

        with col4:
            if st.button(label=sample_question_4):
                question = sample_question_4.lower()

        with col5:
            if st.button(label=sample_question_5):
                question = sample_question_5.lower()

        # Generate the ask question bar
        st.markdown(
            "Type in your question below (Press Ctrl+Enter to key in question):"
        )
    ...
    st.markdown(
        ...
        <div class="footer">
            <a href="https://www.linkedin.com/company/neuron-q/">LinkedIn</a>
